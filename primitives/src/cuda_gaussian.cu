...
__global__ void gaussianHorizontal(float* kernel, int filterSize, const uchar* input, uchar* output, int width,
                                   int height)
{
...
__global__ void gaussianVertical(float* kernel, int filterSize, const uchar* input, uchar* output, int width,
                                 int height)
{
...
GaussianCUDA::GaussianCUDA(std::shared_ptr<CUDAEnv>& deviceEnv, int size, float sigma, int threadsPerBlock)
    : GaussianBase(size, sigma), deviceEnv(deviceEnv), threadsPerBlock(threadsPerBlock)
{
    assert(size <= cKernelSizeMax);
}

GaussianCUDA::GaussianCUDA(int size, float sigma, int threadsPerBlock)
    : GaussianBase(size, sigma), deviceEnv(std::make_shared<CUDAEnv>()), threadsPerBlock(threadsPerBlock)
{
    assert(size <= cKernelSizeMax);
}
...
void GaussianCUDA::apply(const cv::Mat& input, cv::Mat& output)
{
    ...
    if(threadsPerBlock == ThreadsPerBlockDefault && ALIGN_UP(domainSize, 32) >= 2 << 6)
    {
        ...
        while(ALIGN_UP(threadsPerBlock * MAX(input.cols, input.rows) + _kernel.size() * sizeof(float), 4) >
                  sharedMemPerBlock &&
              threadsPerBlock >= 32)
        {
            threadsPerBlock /= 2;
