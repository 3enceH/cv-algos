...
{
    auto elapsed =
        std::chrono::duration_cast<std::chrono::milliseconds>(log.crbegin()->first - log.cbegin()->first).count();

    int strLengthMax = 0;
    for(auto& it : log)
    {
        if(it.second.length() > strLengthMax)
            strLengthMax = (int)it.second.length();
    }
    ...
    while(cur != log.cend())
    {
        ...
        stream << std::setw(strLengthMax) << std::left << cur->second << ": " << std::setw(8) << std::left << diff
               << "ms " << ratio << "%" << std::endl;
        cur++;
        last++;
    }
    stream << std::setw(strLengthMax) << std::left << "SUM"
           << ": " << std::setw(8) << std::left << elapsed << "ms " << 100 << "%" << std::endl;
