...
inline void throwError(const std::string message, const char* fileName, const std::size_t lineNumber)
{
...
#define CHECK(condition, msg)                                                                                          \
    {                                                                                                                  \
        if(!(condition))                                                                                               \
            throwError(msg, __FILE__, __LINE__);                                                                       \
    }
#define CHECK_CUDA(status)                                                                                             \
    {                                                                                                                  \
        if((status) != cudaSuccess)                                                                                    \
            throwError("CUDA error: " + std::string(cudaGetErrorString(status)), __FILE__, __LINE__);                  \
    }
