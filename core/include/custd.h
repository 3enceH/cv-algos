...
namespace custd
{
template<typename Type, size_t Capacity> class vector
{
public:
    ...
    __device__ void pop_back() { _size--; }
...
template<typename Type, size_t Capacity> std::ostream& operator<<(std::ostream& stream, const vector<Type, Capacity>& v)
{
    stream << "vector[";
...
template<typename It> std::ostream& print(std::ostream& stream, It first, It last)
{
    stream << "[";
...
template<typename Key, typename Value> std::ostream& operator<<(std::ostream& stream, const pair<Key, Value>& p)
{
...
template<typename Key, typename Value, size_t Capacity> class map
{
public:
    ...
    __host__ map() : _size(0)
    {
        ...
        CHECK_CUDA(cudaMemcpy((void*)_elems, (void*)buffer, sizeof(pair<Key, Value>) * Capacity,
                              cudaMemcpyKind::cudaMemcpyHostToDevice));
        printf("map allocate %p capacity %llu elemsize %llu bytes %llu\n", _elems, Capacity, sizeof(pair<Key, Value>),
               sizeof(pair<Key, Value>) * Capacity);
    }
    __host__ ~map()
    {
        uint8_t buffer[sizeof(pair<Key, Value>) * Capacity];
        CHECK_CUDA(cudaMemcpy((void*)buffer, (void*)_elems, sizeof(pair<Key, Value>) * Capacity,
